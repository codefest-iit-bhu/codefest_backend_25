...
import { CARequest } from "../models/ca_request";
...
export const getAllRequests = async (req, res, next) => {
    if (req.user.role !== 'admin') return next(new ErrorHandler("You are not allowed to see the CA requests", 403))
...
export const updateRequest = async (req, res) => {
    ...
    if (req.user.role !== 'admin') {
        ...
        if (req.body.userDescription) request.userDescription = req.body.userDescription
    } else {
        if (req.body.adminMessage) request.adminMessage = req.body.adminMessage
    }
    const updatedRequest = await request.save();
    if (!updatedRequest) return next(new ErrorHandler("CA request couldn't be updated", 404));
